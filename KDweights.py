...
def StochasticKd(temp,ans,rebalance_period,window,size):
    '''
    li=[]
    for df in ans:
        li.append(list(df['Close']))
    temp = pd.DataFrame(li).transpose()
    '''
    ...
    for i in range(rebalance_period,size,15):
        ...
        for j in range(0,len(weights)):
            df=ans[j]
            w.append(int(df.loc[i-1,'buy_signal']*greater(weights[j])))
        n_asset=w.count(1.0)
        if(w.count(0.0)==len(weights)):
            w_=[1/len(weights)]*len(weights)
        else:
            w_= [i/n_asset for i in w]
