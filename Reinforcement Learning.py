...
class Agent:
    ...
    def __init__(
        ...
        self.input_shape2 = (portfolio_size, 30, )
...
def mse(l1,l):
        ...
        for i in range (0,n):  #looping through each element of the list
          ...
          squared_difference = difference #taking square of the differene 
...
for e in range(episode_count):
    ...
    s1 = env.get_state2(np.random.randint(window_size+1, data_length-window_size-1), 60)
    total_profit = 0 
...
plt.bar(np.arange(N_ASSETS), a, color = 'black', alpha = 0.25)
plt.xticks(np.arange(N_ASSETS), env.data.columns, rotation='vertical')
plt.legend()
plt.grid()
plt.show()
env.data.columns= ['AAPL', 'AXP', 'BRK', 'C', 'GILD', 'HON', 'INTU',
       'JPM', 'NKE', 'NVDA', 'ORCL', 'PG', 'UAL',
       'WMT', 'XOM']
'''
...
result_rl_CNN_vis = [item for sublist in result_rl for item in sublist]
...
result_rl_CNN_vis = [x+1 for x in result_rl_CNN_vis]
...
d= np.array(result_rl_CNN_vis).cumprod()  
...
plt.plot(df.index,np.array(df).cumsum(),label="S&P 500")
plt.legend(loc="upper left")
plt.xlabel('time period in days')
plt.ylabel('test set returns')
plt.show()

plt.plot(a,label="Max Portfolio Weights")
plt.plot(d,label="RL Portfolio-Return")
plt.plot(c,label="S&P 500")
