# -*- coding: utf-8 -*-
"""
Created on Fri Jul 17 16:31:51 2020
...
for e in range(episode_count):
    ...
    s1 = env.get_state5(np.random.randint(window_size+1, data_length-window_size-1), 60)
    total_profit = 0 
...
a= np.array(result_KD_vis).cumsum() 
b= np.array(result_KDagg_vis).cumsum() 
c= np.array(result_equal_vis).cumsum() 
d= np.array(result_rl_vis).cumsum() 
e= np.array(result_max_vis).cumsum()
